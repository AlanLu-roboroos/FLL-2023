...
class FinalRun(Thread):
    ...
    def run(self):
        ...
        self.drive.moveArc(-400, -80, speed=150)
        self.drive.moveArc(150, -65)
        self.drive.moveDist(190, heading=-65)
        self.drive.turnTo(-90)
        self.drive.moveDist(70, heading=-90, turn=False)
        self.drive.drive.drive(0, 20)
        self.wait(1200)
        self.drive.stop()
        Thread(target=self.runArmAngle, args=[-500, 540]).start()
        self.wait(2000)
        self.drive.moveDist(-150, heading=-90, turn=False)
        Thread(target=self.runArmAngle, args=[600, 580]).start()
        self.drive.moveArc(-162, 180, timeout=4000)

        Thread(target=self.runArmAngle, args=[-400, 530]).start()
        self.drive.moveDist(-180, speed=100, turn=False)
        self.drive.moveDist(210, speed=100, turn=False)
